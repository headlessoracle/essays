...
You might think HTTP already solves this. It nearly does: [RFC 9421, HTTP Message Signatures](https://datatracker.ietf.org/doc/html/rfc9421), lets a server cryptographically sign its response, so an agent can demand signed state instead of a bare value. But read who is doing the signing. The party that signs is the party serving the data — the one with a stake in what the agent does next. A venue, a counterparty, or a vendor signing its own state feed produces a non-repudiable record of what it *claimed*, which is genuinely useful for arguing about fraud after the fact. It does nothing to let an agent decide, in the moment, whether to trust the number before it acts. A signature from an interested party is a promise, not an independent check.
